...
%---------------------------------------------------------------------------------------------------------------------------
% This function initialize the first population of search agents（此函数用于初始化第一批搜索代理）
function X=initialization(nP,dim,ub,lb)

Boundary_no= size(ub,2); % numnber of boundaries（搜索边界）

% If the boundaries of all variables are equal and user enter a signle（如果所有变量的边界相等，用户输入符号）
% number for both ub and lb
if Boundary_no==1
    X=rand(nP,dim).*(ub-lb)+lb;
end

% If each variable has a different lb and ub（如果每个变量有不同的lb和ub）
if Boundary_no>1
    for i=1:dim
        ub_i=ub(i);
        lb_i=lb(i);
